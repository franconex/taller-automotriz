...
@section('content')
    <x-admin.page-header title="Editar proveedor" :description="'Modifica los datos de ' . $proveedor->nombre_empresa . '.'">
        <x-slot:actions>
    ...
    <div class="admin-card-modern p-4">
        <form method="POST" action="{{ route('admin.proveedores.update', $proveedor) }}">
            @csrf @method('PUT')
            @include('admin.proveedores._form', ['proveedor' => $proveedor, 'codigo_pais' => $codigo_pais ?? '+591', 'telefono_numero' => $telefono_numero ?? ''])
